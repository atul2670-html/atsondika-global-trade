  ...
  const [heroBanner, setHeroBanner] = useState(() => {
    try {
      ...
      if (stored && stored.title) {
        ...
        // Guarantee all languages always contain full "Textile Products" ("પ્રોડક્ટ્સ (કાપડ ઉત્પાદનો)", "कपड़ा उत्पादों", "Produits Textiles")
        if (!titleObj.gu || titleObj.gu.includes('ટેક્સટાઇલ,') || !titleObj.gu.includes('ટેક્સટાઇલ પ્રોડક્ટ્સ')) {
          titleObj.gu = defaultHeroBanner.title.gu;
        }
        if (!titleObj.hi || titleObj.hi.includes('कपड़ा,') || !titleObj.hi.includes('कपड़ा उत्पाद')) {
          titleObj.hi = defaultHeroBanner.title.hi;
        }
        if (!titleObj.fr || titleObj.fr.includes('textiles,') || !titleObj.fr.includes('produits textiles')) {
          titleObj.fr = defaultHeroBanner.title.fr;
        }
        if (!titleObj.en || !titleObj.en.includes('Textile Products')) {
          titleObj.en = defaultHeroBanner.title.en;
        }
